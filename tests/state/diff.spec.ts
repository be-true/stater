...
describe('State#diff', () => {
  it('Простое сравнивание. Опции не передаются.', () => {
    ...
    expect(result).toEqual({
      ...
      colors: { f: { front: 'white', back: 'black' }, s: { front: 'white', back: 'red' } },
      pets: { f: [{ name: 'Шарик' }, { name: 'Мурзик' }], s: [{ name: 'Кузя' }, { name: 'Мурзик' }] },
  ...
  it('Опции передаются { skipNew: true } НЕ учитывать новые свойства.', () => {
    ...
    const result = state.diff({ name: 'Дмитрий', lastName: 'Донской', newProperty: 'newProperty', pets: [{ name: 'Шарик' }] }, { skipNew: true });
  ...
  it('Опции передаются { skipOld: true } НЕ учитывать старые свойства.', () => {
    const data = { name: 'Евгений', lastName: 'Онегин', test: 'test', pets: [{ name: 'Шарик' }] };
    const state = new State<typeof data>(data).source('any_source').asNew();

    const result = state.diff({ name: 'Дмитрий', lastName: 'Донской', newProperty: 'newProperty' }, { skipOld: true });
  ...
  it('Опции передаются { skipOld: true } и { skipNew: true } НЕ учитывать старые И новые свойства.', () => {
    ...
    const result = state.diff(
      { name: 'Дмитрий', lastName: 'Донской', newProperty: 'newProperty', pets: [{ name: 'Шарик' }] },
  ...
  it('Принимаемые параметры не верны. Изначальное состояние - строка', () => {
    const data = { name: 'Дмитрий', lastName: 'Донской', newProperty: 'newProperty' };
    const state = new State<typeof data>(data).source('any_source').asNew();

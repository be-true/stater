...
describe('StateList#subList()', () => {
  it('Получение через callback', () => {
    ...
    const fullList = new StateList<{ name: string; age: number }>([state1, state2, state3]).source('any_source');
    ...
    expect(subList.getList().map((i) => i.get('name'))).toEqual(['Alice', 'Eugen', 'Morgen']);
    // В основной список он тоже добавляется
    expect(fullList.getList().map((i) => i.get('name'))).toEqual(['Bob', 'Alice', 'Eugen', 'Morgen']);
